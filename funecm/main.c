...
main (int argc, char *argv[])
{
  ...
  int window_size = 4;
  int montgomery_flag = 0;
  int atkin_flag = 0;
  while ((opt = getopt (argc, argv, "hlc:w:am")) != -1)
    {
      switch (opt)
	{
	case 'h':
	  fprintf (stdout,
		   "Usage: funecm [options] <composite number> <B1> <B2> <filename>\n");
	  ...
	  fprintf (stdout, "-m: use Montgomery\n");
	  return 0;
	  break;
	case 'l':
	  loop = 1;
	...
	case 'c':
	  ...
	  if (number_of_elliptic_curves == 0)
	    {
	      fprintf (stderr,
		       "Error: the argument of 'c' option must be number\n");
	      return 1;
	    }
	  break;
	case 'w':
	  ...
	  if (window_size <= 1 || 31 <= window_size)
	    {
	      fprintf (stderr,
		       "Error: the argument of 'w' option must be [2-30]\n");
	      return 1;
	    }
	  break;
	case 'a':
	...
	if (atkin_flag)
	...
	else if (montgomery_flag)
	  {
	    ...
	    while (mpz_cmp_ui (Y, 2) < 0)
	      mpz_add_ui (Y, Y, 1);
	    while (mpz_cmp_ui (montgomery_a, 2) == 0)
	...
	if (found == 0)
	  {
		...
		ecm (factor, N, X, Y, d, B1, B2, fp, window_size);
	      ...
	      {
		Montgomery_ecm (factor, N, NULL, Y, d, montgomery_a, B1, B2,
				fp, window_size);
	      }
	    else
	      {
		...
		mpz_clears (X, Y, d, montgomery_a, montgomery_b, NULL);
		mpz_clear (factor);
		mpz_clear (cofactor);
		continue;
	      }
...
//
	    ...
	    switch (mpz_probab_prime_p (factor, 25))
	      ...
	      case 2:
		gmp_fprintf (fp,
			     "@ definite prime factor found: %Zd  digits: %d cofactor: %Zd\n\n",
			     factor, strlen (digits), cofactor);
		break;
	      case 1:
		gmp_fprintf (fp,
			     "@ probable prime factor found: %Zd  digits: %d cofactor: %Zd\n\n",
			     factor, strlen (digits), cofactor);
		break;
	      case 0:
		gmp_fprintf (fp,
			     "@ composite prime factor found: %Zd  digits: %d cofactor: %Zd\n\n",
			     factor, strlen (digits), cofactor);
		break;
	      default:
		break;
