...
void
ecm (mpz_t f, const mpz_t N, const mpz_t X, const mpz_t Y, mpz_t d,
     const unsigned long int B1, const unsigned long int B2, FILE * fp,
...
void
Montgomery_ecm (mpz_t f, const mpz_t N, const mpz_t X, const mpz_t Y, mpz_t d,
		const mpz_t montgomery_a, const unsigned long int B1,
		const unsigned long int B2, FILE * fp, const int window_size)
{
  PROJECTIVE_POINT P;
  int e;
  int i;
  ...
  montgomery_bsgs (f, P, B1, B2, window_size, N, fp, d, montgomery_a,
		   montgomery_b);
  end = omp_get_wtime ();
  stage2_time = end - start;
