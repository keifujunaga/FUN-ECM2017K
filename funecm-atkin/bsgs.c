...
void bsgs(mpz_t p,PROJECTIVE_POINT P,const unsigned long int B1,const unsigned long int B2,const int window_size,const mpz_t N,const FILE *fp,const mpz_t D){
 ...
 // }
  ...
  for(i=0;i<f4;i++){
    mpz_init(baby_step[i]);
//worn    mpz_set_ui(ti,2*i+1);
    //mpz_gcd(gcd,ti,tf);
    //if(mpz_cmp_ui(gcd,1)==0){
     // extended_point_set(baby_step[i],add_eP);
      //}
    //extended_point_set(baby_step[i],eP);
    //mpz_invert(baby_step[i],eP->Z,N);
    mpz_invert(baby_step[i],add_eP->Z,N);
    mpz_mul_mod(baby_step[i],baby_step[i],add_eP->Y,N);
...
//     mpz_mul_mod(product,Giant->Y,baby_step[(u-1)/2]->Z,N);
     mpz_mul_mod(product,Giant->Z,baby_step[(u-1)/2],N);
     mpz_sub(product,product,Giant->Y);
  ...
  mpz_clears(ts,d,product,NULL);
  extended_point_clear(eP);
  extended_point_clear(add_eP);
  for(i=0;i<f4;i++){
    //extended_point_clear(baby_step[i]);
    mpz_clear(baby_step[i]);
  }
